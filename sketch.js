   ...
   var shield, shI, tI, tryAgain, sI, endGame, gameOver;
   ...
   var life = 3;
   var startGame;
   var storySound, extraLife, lossHeart, obstacleHit, fail, laser, chS;
 ...
 function setup(){
     ...
     tryAgain = createSprite(windowWidth/2, windowHeight/2 + 100, 20, 20);
 ...
 function draw(){
  ...
  if(gameState === STORY){
     ...
     textSize(30);
     fill("yellow");
     text("Press Space to Skip Intro", width - 500, height - 25);
...
if(keyDown("space")){
  ...
  if(frameCount >= 2200 && frameCount <= 4200){
     scrollBackground2();
}    
     
  if(frameCount > 4200 && frameCount <= 6200) {
     scrollBackground3();   
}
 ...
 if(frameCount > 12200 && frameCount <= 14200){
     scrollBackground3();   
}
 ...
 function spawnObstacles(){
  if(frameCount % 160 === 0){
   var obstacle1 = createSprite(windowWidth, 50, 20, 20);
     obstacle1.velocityX = random(-5, -20);
